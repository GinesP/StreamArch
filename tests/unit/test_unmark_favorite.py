"""Tests for UnmarkFavoriteHandler."""
...
def _insert_target(repo: StreamTargetRepository, **overrides) -> str:
    ...
    target = StreamTarget(
        ...
        favorite=overrides.get("favorite", True),
...
@pytest.fixture
def handler(repo: StreamTargetRepository) -> UnmarkFavoriteHandler:
    return UnmarkFavoriteHandler(stream_target_repo=repo)


class TestUnmarkFavoriteHandler:
    def test_unmarks_favorite(self, handler, repo) -> None:
        tid = _insert_target(repo, id="t1", favorite=True)

        handler.handle(UnmarkFavoriteCommand(stream_id="t1"))
        updated = repo.get("t1")

        assert updated is not None
        assert updated.favorite is False

    def test_idempotent_when_already_not_favorite(self, handler, repo) -> None:
        tid = _insert_target(repo, id="t2", favorite=False)
        before = repo.get(tid).updated_at

        handler.handle(UnmarkFavoriteCommand(stream_id="t2"))
        after = repo.get(tid).updated_at

        assert repo.get(tid).favorite is False
        assert after == before  # No update performed

    def test_raises_on_missing_target(self, handler) -> None:
        cmd = UnmarkFavoriteCommand(stream_id="nonexistent")
        with pytest.raises(ValueError, match="not found"):
            handler.handle(cmd)

    def test_updates_updated_at_timestamp(self, handler, repo) -> None:
        tid = _insert_target(repo, id="t3", favorite=True)
        before = repo.get(tid).updated_at

        handler.handle(UnmarkFavoriteCommand(stream_id="t3"))
        after = repo.get(tid).updated_at
